<template>
  <div>
    ...
    <a-form ref="searchRef" :model="search.data" @finish="onFinishSearch">
      <a-row :gutter="24">
        <a-col :span="8">
          <a-form-item name="name" label="权限组名称">
            <a-input
    ...
    <div class="h-10 leading-10">
      <a-button
        v-if="userStore.permissionCodeMap['security:permission-group:add']"
        type="primary"
        @click="onClickAddBtn"
      >
        新增
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'operation'">
          <div class="space-x-4">
            <a
              v-if="
                userStore.permissionCodeMap['security:permission-group:view']
              "
            ...
            <a
              v-if="
                userStore.permissionCodeMap['security:permission-group:edit']
              "
            ...
            <a-popconfirm
              v-if="
                userStore.permissionCodeMap['security:permission-group:delete']
              "
              title="确认删除吗？"
              ok-text="确认"
              cancel-text="取消"
...
const table = ref({
  loading: false,
  columns: [
    { title: "权限组名称", dataIndex: "name", width: 160 },
    { title: "权限组编码", dataIndex: "code", width: 160 },
...
const getTableData = () => {
  ...
  return apisSecurityPermissionGroup
    .getPermissionGroupPage({
      ...query,
...
const onClickViewLink = (row) => {
  apisSecurityPermissionGroup.getPermissionGroupById(row.id).then((res) => {
    crudModal.value = {
      open: true,
      type: "view",
      data: res.data,
...
const onConfirmDeleteRow = (row) => {
  apisSecurityPermissionGroup.deletePermissionGroupById(row.id).then((res) => {
    return getTableData();
  });
};
